...
static void move_test_sprite(void)
{
	...
	const uint16_t attr_base = VDP_ATTR(VDP_TILE(TEST_SPR_VRAM_ADDR),
	                           ...
	                           TEST_SPR_PAL,
	                           /*prio=*/false);
	sai_vdp_spr_draw(s_x,          s_y,          attr_base+(SPR_C1_FRAME_OFFS*0), SPR_C1_SIZE);
	sai_vdp_spr_draw(s_x+SPR_C1_W, s_y,          attr_base+(SPR_C1_FRAME_OFFS*1), SPR_C1_SIZE);
	sai_vdp_spr_draw(s_x,          s_y+SPR_C1_H, attr_base+(SPR_C1_FRAME_OFFS*2), SPR_C1_SIZE);
	sai_vdp_spr_draw(s_x+SPR_C1_W, s_y+SPR_C1_H, attr_base+(SPR_C1_FRAME_OFFS*3), SPR_C1_SIZE);
}
