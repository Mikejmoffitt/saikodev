...
static void move_test_sprite(void)
{
	...
	sai_vdp_spr_draw(s_x+128,          s_y+128,          attr_base+(SPR_C1_FRAME_OFFS*0), SPR_C1_SIZE);
	sai_vdp_spr_draw(s_x+128+SPR_C1_W, s_y+128,          attr_base+(SPR_C1_FRAME_OFFS*1), SPR_C1_SIZE);
	sai_vdp_spr_draw(s_x+128,          s_y+128+SPR_C1_H, attr_base+(SPR_C1_FRAME_OFFS*2), SPR_C1_SIZE);
	sai_vdp_spr_draw(s_x+128+SPR_C1_W, s_y+128+SPR_C1_H, attr_base+(SPR_C1_FRAME_OFFS*3), SPR_C1_SIZE);
#else
	g_sai_vdp_spr[0].x = s_x+128;
	g_sai_vdp_spr[0].y = s_y+128;
	g_sai_vdp_spr[0].attr = attr_base+(SPR_C1_FRAME_OFFS*0);
	g_sai_vdp_spr[0].size = SPR_C1_SIZE;
	g_sai_vdp_spr[0].link = 1;
	g_sai_vdp_spr[1].x = s_x+128+SPR_C1_W;
	g_sai_vdp_spr[1].y = s_y+128;
	g_sai_vdp_spr[1].attr = attr_base+(SPR_C1_FRAME_OFFS*1);
	g_sai_vdp_spr[1].size = SPR_C1_SIZE;
	g_sai_vdp_spr[1].link = 2;
	g_sai_vdp_spr[2].x = s_x+128;
	g_sai_vdp_spr[2].y = s_y+128+SPR_C1_H;
	g_sai_vdp_spr[2].attr = attr_base+(SPR_C1_FRAME_OFFS*2);
	g_sai_vdp_spr[2].size = SPR_C1_SIZE;
	g_sai_vdp_spr[2].link = 3;
	g_sai_vdp_spr[3].x = s_x+128+SPR_C1_W;
	g_sai_vdp_spr[3].y = s_y+128+SPR_C1_H;
	g_sai_vdp_spr[3].attr = attr_base+(SPR_C1_FRAME_OFFS*3);
	g_sai_vdp_spr[3].size = SPR_C1_SIZE;
...
void __attribute__((noreturn)) main(void)
{
	...
	sai_pal_load(TEST_SPR_PAL, vel_get_wrk_gfx_pal(SPR_C1), 1);
	sai_vdp_dma_transfer_vram(TEST_SPR_VRAM_ADDR, vel_get_wrk_gfx_chr(SPR_C1),
	                          SPR_C1_CHR_WORDS, 2);
